...
def _reject_nonfinite(value: Any, path: str = "inputs") -> None:
    """Fail closed before JSON can carry NaN or Infinity."""
    if isinstance(value, dict):
        for key, item in value.items():
            _reject_nonfinite(item, str(key))
