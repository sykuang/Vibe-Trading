...
Vibe-Trading exposes 60 MCP tools for any MCP-compatible client. Runs as a stdio subprocess — no server setup needed. Core research tools work with zero API keys for HK/US/crypto; trading connector tools use the selected connector profile, and `run_swarm` needs an LLM key.
...
│   ├── mcp_server.py               # MCP server — 60 tools for OpenClaw / Claude Desktop
│   │
│   ├── src/
